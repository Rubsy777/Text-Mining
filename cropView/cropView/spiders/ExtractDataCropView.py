...
class ExtractDataCropView(object):
    ...
    def checkDict(infoList, foodDictionary, farmDictionary, constructionDictionary, medicinalDictionary, productionDictionary, IDlist, nameList):
        ...
        with open('cropViewData.csv', 'wb') as csvfile:
            fieldnames = ['id', 'Name', 'MinHeight', 'MaxHeight', 'GrowingPeriod', 'Uses', 'MinProduction', 'MaxProduction']
            ...
            for x in infoList[1:]:
                ...
                for dictWord in foodDictionary:
                    if re.search(dictWord, paragraph, re.IGNORECASE): 
                ...
                if re.search('annual', paragraph, re.IGNORECASE): 
                    Growing.append('Annual') 
                                    
                if re.search('perennial', paragraph, re.IGNORECASE): 
                    Growing.append('Perennial')
                                    
                if re.search('biennial', paragraph, re.IGNORECASE): 
                ...
                for dictWord in productionDictionary:
                    if re.search(dictWord, paragraph, re.IGNORECASE): 
                        productionPos = paragraph.find(dictWord)
                        productionDescription = parser.parse(paragraph[productionPos-20:productionPos+26])
                ...
                heightPos = paragraph.find("height")
                heightDescription = parser.parse(paragraph[heightPos-20:heightPos+26])
                ...
                writer.writerow({'id':IDlist[y], 'Name':nameList[y], 'MinHeight':heightDescriptio0, 'MaxHeight':heightDescriptio1, 'GrowingPeriod':GrowingDescription, 'Uses':usesDescription,'MinProduction':productionDescription0, 'MaxProduction':productionDescription1})
